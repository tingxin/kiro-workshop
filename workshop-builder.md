...
### 3.3 课堂内确认的 Workshop 模拟决策

原始需求中的阈值和优先级不能预先作为学员答案。课程必须先由学员使用 Analyze Requirements 找出阻塞项，再由产品、架构、测试、安全等角色在课堂讨论中逐条确认模拟决策，最后由学员使用 Kiro 创建 Decision Log。

默认学员工作区不得预置完成的 Decision Log 或 Feature Spec。讲师答案只能用于私下评分、逐条口头释放决定或 Kiro 完全不可用时的隔离备用，不得作为学员 Agent 上下文。

课堂确认至少要覆盖以下决策类别，但具体结论必须来自人工确认：

- V1 订单范围和旧流程边界；
- 时间、距离、金额单位及边界包含关系；
- 免费取消、验证到达和爽约门控；
- 法律、安全、无障碍及关键依赖异常的优先级；
- 费用调整、减免、非负和封顶顺序；
- 收费幂等、降级、解释、审计和敏感证据可见性；
- 仍待确认和明确排除的生产事项。

所有课堂结论必须标注为 Workshop 模拟决策，并通过 Decision ID 追溯到讨论记录和原始需求章节。
...
| Analyze Requirements | 找出歧义、冲突、缺失和不可验收描述 | 学员现场完成并保存分析产物；不得用预制答案替代正常路径 |
...
核心 Workshop 目标时长约 90 分钟，用于让学员亲自完成从 Starter 到费用纵切的最小闭环；不得通过预置 Decision Log、Feature Spec 或完成代码压缩步骤。若目标是让学员继续实现已批准 V1 Spec 的参考系统，则使用 `workshop/03-hands-on-lab-guide.md` 的多阶段扩展 Lab，不受 90 分钟限制。

`custom-find-skill` 应嵌入现有主线，而不是额外形成一场独立 Workshop。具体时间可以根据现场讨论调整。
...
4. Analyze Requirements 和 Property Testing 在正常路径中由学员现场完成到当前 Lab 所需的最小范围；预制结果只能作为讲师隔离应急材料；
